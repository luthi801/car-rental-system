...
class admin_content_controller extends Controller
{
    ...
    public function index(){
        ...
        $w_order = Order::where('vendor_name', 'RentNow')->get();
        ...
        $totalPrice = $order_data->sum('total_price');
        
        // Calculate RentNow's direct sales
        $w_Price = $w_order->sum('total_price');
        
        // Calculate vendor sales and commission
        $vp_sale = $order_data->where('vendor_name', '!=', 'RentNow')->sum('total_price');
        $vp_sale3 = $vp_sale * 0.10; // 10% commission
          
        return view('adminpanel.admin_dashboard', compact(
            'order_data',
            'product_data',
            'cus_data',
            'w_order',
